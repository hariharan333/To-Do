                  ...
                  <div class="card-body">
                      <form v-on:submit.prevent>
                        <div class="input-group">
                          ...
                          <span class="input-group-btn">
                            <button class="btn custom-button1" :class="{'btn-success' : !enableEdit, 'btn-warning' : enableEdit}" type="button"  v-on:click="searchTodo"><span :class="{'fa fa-search' : enableSearch,}"></span></button>
                          </span>
                        </div>
                      </form>
                      <ul class="list-group">
        ...
        methods: {
          ...
          searchTodo(){
            ...
            if(this.enableSearch)
              {
                 console.log("hello")
                  this.$http.get('todo/'+this.todo.title).then(response => {
                  if(response.status == 200){
          ...
          editTodo(todo, todoIndex){
            if(todo.completed == true && completedToggle == true){
              this.enableEdit = false;
            }else{
